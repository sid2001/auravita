...
def connection_request_callback(
    ...
    """
    doctor = user_collection.find_one({"user_type":"doctor","_id": ObjectId(doctor_id)}) # doctor
    patient = user_collection.find_one({"user_type":"patient" , "_id": ObjectId(patient_id)}) # patient
    
    connection_request = db.connection_requests.find_one({"doctor_id": ObjectId(doctor_id), "patient_id": ObjectId(patient_id)})
...
def reject_connection_request_callback(session, connection_request_id: str):
    ...
    doctor_id = connection_request["doctor_id"] #type = ObjectId
    patient_id = connection_request["patient_id"] #type = ObjectId
